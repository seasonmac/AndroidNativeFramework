...
#include <zip_archive_stream_entry.h>
#include <zlib.h>
...
bool ZipArchiveStreamEntry::Init(const ZipEntry& entry) {
  ...
  if (!archive->mapped_zip.SeekToOffset(data_offset)) {
    HLOGW("lseek to data at %" PRId64 " failed: %s", data_offset, strerror(errno));
    return false;
  }
  crc32_ = entry.crc32;
  return true;
...
const std::vector<uint8_t>* ZipArchiveStreamEntryUncompressed::Read() {
  ...
  if (!archive->mapped_zip.ReadData(data_.data(), bytes)) {
    if (errno != 0) {
      HLOGE("Error reading from archive fd: %s", strerror(errno));
    } else {
      HLOGE("Short read of zip file, possibly corrupted zip?");
    }
    length_ = 0;
    return nullptr;
  }
...
bool ZipArchiveStreamEntryCompressed::Init(const ZipEntry& entry) {
  ...
  if (zerr != Z_OK) {
    if (zerr == Z_VERSION_ERROR) {
      ...
      HLOGE("Call to inflateInit2 failed (zerr=%d)", zerr);
    }
...
const std::vector<uint8_t>* ZipArchiveStreamEntryCompressed::Read() {
  ...
  while (true) {
    ...
    if (zerr != Z_OK && zerr != Z_STREAM_END) {
      HLOGE("inflate zerr=%d (nIn=%p aIn=%u nOut=%p aOut=%u)", zerr, z_stream_.next_in,
